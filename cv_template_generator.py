...
WORD_COUNT_MIN = 35  # More flexible range to accommodate AI variations
WORD_COUNT_MAX = 65  # More flexible range to accommodate AI variations
COMPETENCIES_COUNT = 9
MAX_RETRIES = 2  # Increased retries for better success rate
...
def generate_cv_content(
    ...
    try:
        ...
        if language.lower() == 'en':
            ...
            lang_instruction = """Schreibe alle Inhalte auf Deutsch mit korrekter Grammatik und Rechtschreibung. 
Verwende 'ss' statt 'ß' (z.B. 'muss' statt 'muß', 'grosse' statt 'große').
Achte auf korrekte Satzkonstruktion, Wortstellung und Zeichensetzung."""
...
1. **cv_motivation** (45-55 Wörter): Ein überzeugendes Motivationsstatement, das erklärt, warum 
   der Kandidat an dieser spezifischen Position bei diesem Unternehmen interessiert ist. 
   Soll persönlich, enthusiastisch sein und Wissen über das Unternehmen/die Rolle zeigen.

2. **cv_kurzprofil** (45-55 Wörter): Ein kurzes professionelles Profil, das die relevantesten 
   Qualifikationen, Erfahrungen und Stärken des Kandidaten für DIESE spezifische Position hervorhebt. 
   Fokus darauf, was ihn/sie zu einem starken Kandidaten macht.

3. **fachkompetenzen** (genau 9 Einträge): Eine Liste der relevantesten technischen und 
   fachlichen Kompetenzen des Kandidaten für diese Position. Jeder Eintrag sollte 2-5 Wörter umfassen. 
   Beispiele: "Python, FastAPI, Django", "Cloud-Architektur (AWS)", "Agile Teamführung"

## Wichtige Anforderungen:
- {lang_instruction}
- Die Wortzahl MUSS strikt zwischen 45-55 Wörtern für cv_motivation und cv_kurzprofil liegen
- fachkompetenzen MUSS genau 9 Einträge enthalten
- Der Inhalt muss spezifisch für DIESE Stelle sein, nicht generisch
- Verwende "ss" statt "ß" im deutschen Text
- Bewahre einen professionellen Ton
...
{{
  "cv_motivation": "Dein 45-55 Wörter Motivationstext hier",
  "cv_kurzprofil": "Dein 45-55 Wörter Profiltext hier",
  "fachkompetenzen": [
    "Kompetenz 1",
    "Kompetenz 2",
    "Kompetenz 3",
...
def render_cv_template(
    ...
    try:
        ...
        # Format fachkompetenzen as bullet points
        fachkompetenzen_list = content.get('fachkompetenzen', [])
        if isinstance(fachkompetenzen_list, list):
            # Join with bullet points and newlines
            fachkompetenzen_text = '\n'.join([f'• {item}' for item in fachkompetenzen_list])
        else:
            fachkompetenzen_text = str(fachkompetenzen_list)
        
        # Prepare context for rendering
        context = {
            'cv_motivation': content.get('cv_motivation', ''),
            'cv_kurzprofil': content.get('cv_kurzprofil', ''),
            'fachkompetenzen': fachkompetenzen_text
        }
