...
def send_to_queue():
    """
    Send selected job matches to the application queue.
    
    Expected JSON payload:
    {
        "match_file": "job_matches_TIMESTAMP.json",
        "selected_indices": [0, 2, 5]  # Indices of selected matches
    }
    
    Returns JSON response with success/error status and details.
    """
    try:
        from services.queue_bridge import QueueBridgeService
        ...
        if not match_file:
            return jsonify({
                'success': False,
                'error': 'Missing match_file parameter'
            }), 400
        ...
        # Secure the filename and ensure .json extension
        secure_match_file = secure_filename(match_file)
        # Convert .md to .json if needed
        if secure_match_file.endswith('.md'):
            secure_match_file = secure_match_file.replace('.md', '.json')
        
        match_file_path = Path(current_app.root_path) / 'job_matches' / secure_match_file

        # Check if match file exists
        if not match_file_path.is_file():
            return jsonify({
                'success': False,
                'error': f'Match file not found: {secure_match_file}'
            }), 404
        ...
        # Validate indices
        invalid_indices = [i for i in selected_indices if i < 0 or i >= len(all_matches)]
        if invalid_indices:
            return jsonify({
                'success': False,
                'error': f'Invalid match indices: {invalid_indices}'
            }), 400

        # Initialize bridge service with root path
        ...
        for idx in selected_indices:
            ...
            company_name = match.get('company', 'Unknown Company')
            ...
            try:
                ...
                if pending_dir.exists():
                    for pending_file in pending_dir.glob('application-*.json'):
                        try:
                            with open(pending_file, 'r', encoding='utf-8') as f:
                                existing_app = json.load(f)
                            
                            # Check for duplicate (same company + title)
                            if (existing_app.get('company_name', '').lower() == company_name.lower() and
                                existing_app.get('job_title', '').lower() == job_title.lower()):
                                is_duplicate = True
                                results['duplicate_count'] += 1
                                results['duplicates'].append({
                                    'job_title': job_title,
                                    'company': company_name,
                                    'existing_file': pending_file.name
                                })
                                logger.warning(f"Duplicate detected: {company_name} - {job_title}")
                                break
                        except Exception as e:
                            logger.error(f"Error checking duplicate in {pending_file}: {e}")
                            continue

                if is_duplicate:
                    continue  # Skip this application

                # Send to queue using bridge service
                application_id = bridge.send_to_queue(match)
                
                if application_id:
                    results['success_count'] += 1
                    results['queued_applications'].append({
                        'id': application_id,
                        'job_title': job_title,
                        'company': company_name
                    })
                    logger.info(f"Successfully queued application {application_id}: {company_name} - {job_title}")
                else:
                    results['error_count'] += 1
                    results['errors'].append({
                        'job_title': job_title,
                        'company': company_name,
                        'error': 'Failed to create queue application (bridge returned None)'
                    })
                ...
                logger.error(f"Error queuing application for {company_name} - {job_title}: {error_msg}", exc_info=True)

        # Prepare response
        response = {
            'success': results['success_count'] > 0 or results['error_count'] == 0,
            'message': f"Queued {results['success_count']} application(s)",
            'results': results
        }

        # Add warning messages for duplicates and errors
        if results['duplicate_count'] > 0:
            response['message'] += f", {results['duplicate_count']} duplicate(s) skipped"
        if results['error_count'] > 0:
            response['message'] += f", {results['error_count']} error(s)"
            response['success'] = results['success_count'] > 0  # Partial success if some succeeded

        return jsonify(response), 200

    except Exception as e:
        logger.error(f'Error in send_to_queue route: {str(e)}', exc_info=True)
        return jsonify({
            'success': False,
