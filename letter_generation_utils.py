...
# Removed @handle_exceptions to return specific error info
@log_execution_time()
def generate_motivation_letter(cv_summary, job_details):
    ...
    Returns:
        Tuple (bool, dict | str): (True, result_dict) on success, (False, error_message) on failure.
        result_dict contains 'motivation_letter_json', 'motivation_letter_html',
        'html_file_path', 'json_file_path', 'scraped_data_path'.
    """
    try:
        ...
        # --- Log the CV summary being used ---
        logger.info(f"CV Summary provided for letter generation (first 200 chars): '{cv_summary[:200]}...'")
        if not cv_summary or not cv_summary.strip():
             logger.error("CV Summary is empty or None before prompt formatting! Aborting.")
             # This check is redundant if the calling function already checks, but added for safety.
             return False, "CV Summary was empty."
        # --- End Log ---

        # Updated prompt: Include Contact Person in input, request contact_person in output JSON.
        prompt = """
        DEINE WICHTIGSTE AUFGABE: Erstelle ein Motivationsschreiben, das die Informationen aus dem Lebenslauf des Bewerbers DIREKT mit den Anforderungen der Stellenbeschreibung verknüpft. Der Hauptteil des Schreibens MUSS zeigen, wie die spezifischen Erfahrungen und Fähigkeiten des Bewerbers (aus dem Lebenslauf) die Anforderungen der Stelle (aus der Stellenbeschreibung) erfüllen. IGNORIERE DIESE ANWEISUNG NICHT!

        Schreibe das Motivationsschreiben aus den Informationen von der Webseite IN DER GLEICHEN SPRACHE WIE DIE INFORMATIONEN VON DER WEBSEITE! und dem Lebenslauf:
        ...
        system_prompt = "You are a professional job application consultant who creates motivation letters. Synthesize information from BOTH the provided CV summary AND the job details to create a compelling letter. Always use the SAME LANGUAGE as the job description provided in the prompt. Adapt your writing style and language to match the job posting language perfectly."
        motivation_letter_json = generate_json_from_prompt(
            prompt=prompt,
            system_prompt=system_prompt, # Updated system prompt
            default=None # Return None on failure from helper now
        )

        # Check if the helper function failed (returned None)
        if motivation_letter_json is None:
            # Error is already logged by generate_json_from_prompt or its underlying calls
            return False, "Failed to generate structured data from OpenAI API."
        ...
        # Return success tuple with dictionary of results and paths
        return True, {
        ...
        logger.error(f"Unexpected error during motivation letter generation: {e}", exc_info=True)
...
def generate_email_text_only(cv_summary, job_details):
    ...
    Returns:
        Tuple (bool, str | None): (True, email_text) on success, (False, error_message) on failure.
    """
    try:
        ...
        email_json = generate_json_from_prompt(
            ...
            default=None # Return None on failure from helper
        )

        if email_json is None:
            # Error logged by helper
            return False, "Failed to generate email text data from OpenAI API."
        ...
        if email_text:
            ...
            return False, "Generated data was missing the required email text."
    except Exception as e:
        logger.error(f"Unexpected error during email text generation: {e}", exc_info=True)
        return False, f"An unexpected error occurred: {e}"
